...
data class HomeSubjectSystemBean(
    ...
    @SerializedName("unit")
    val unit1: String, // 价格---单价

    @SerializedName("banner_type")
    val banner_type: String, // 卡片底图类型

    @SerializedName("banner_list")
    val banner_list: List<HomeSubjectExperienceBannerBean>? = null, // 卡片头图列表
    ...
    override fun getOpenCourseTime(): String {
        try {
            var time: Long = open_course_time.toLong();
            time *= 1000
    ...
    override fun getBannerImgList(): List<HomeSubjectExperienceBannerBean>? {
        if (banner_list != null && banner_list.isNotEmpty()) {
            val imgList: ArrayList<HomeSubjectExperienceBannerBean> = ArrayList()

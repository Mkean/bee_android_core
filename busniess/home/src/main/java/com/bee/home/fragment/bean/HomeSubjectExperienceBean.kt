...
data class HomeSubjectExperienceBean(
    ...
    val open_course_time: String, // 开课时间：单位 秒

    @SerializedName("sale_point_name")
    val sale_point_name: String, // 买点说明 "适合幼小衔接"
    ...
    val sale_user_list: List<HomeSubjectCardSaleUserBean> // 已经购买用户数组
     ...
     * 获取下一个销售时间
     *
     * @return 0 则代表服务器未返回时间
     */
    fun getSaleStartTime(): Long {

        return try {
            sale_start_time.toLong() * 10000
        } catch (e: Exception) {
            e.printStackTrace()
            0
        }
    ...
    override fun getUnit(): String = unit1
    ...
    override fun getBannerVideoBean(): HomeSubjectExperienceBannerBean? {
        if (banner_list != null && banner_list.isNotEmpty()) {
            for (bannerBean in banner_list) {
                if (bannerBean.getBannerType() == banner_type_video) {
                    return bannerBean
                }
            }
        }
    ...
    override fun getBannerImgList(): List<HomeSubjectExperienceBannerBean>? {
        if (banner_list != null && banner_list.isNotEmpty()) {
            ...
            for (bannerBean in banner_list) {
                if (bannerBean.getBannerType() == banner_type_image) {
